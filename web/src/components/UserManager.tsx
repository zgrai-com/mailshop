  ...
  return (
    <section className="account-view">
      ...
      <section className="account-table-section">
        ...
        {loading ? <div className="page-loading"><LoaderCircle className="spin" size={22} />加载账号</div> : <div className="table-scroll"><table className="data-table accounts-table"><thead><tr><th>员工</th><th>账号</th><th>最近登录</th><th>状态</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><div className="user-cell"><span className="avatar-small">{user.displayName.slice(0, 1).toUpperCase()}</span><div><strong>{user.displayName}</strong>{user.id === currentUser.id && <small>当前账号</small>}</div></div></td><td className="mono">{user.username}</td><td>{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString("zh-CN") : "从未登录"}</td><td><label className="toggle-control"><input type="checkbox" checked={user.isActive !== false} disabled={user.id === currentUser.id} onChange={(event) => onPatch(user.id, { isActive: event.target.checked })} /><span aria-hidden="true" /><em>{user.isActive !== false ? "启用" : "停用"}</em></label></td><td className="actions-cell"><button className="button quiet compact" type="button" onClick={() => setResetUser(user)}><KeyRound size={15} />重置密码</button></td></tr>)}</tbody></table></div>}
      </section>
