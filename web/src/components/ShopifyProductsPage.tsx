...
function listReturnPath(state: { storeId: string; search: string; status: string; inventory: string; productType: string; vendor: string; sortKey: string; reverse: boolean; pageSize: number; page: number }): string {
  return "/shopify/products" + toQuery({ storeId: state.storeId, search: state.search, status: state.status, inventory: state.inventory, productType: state.productType, vendor: state.vendor, sortKey: state.sortKey, reverse: String(state.reverse), first: state.pageSize, page: state.page > 1 ? state.page : undefined });
}
...
export function ShopifyProductsPage({ stores, onError, onNotify, onOpenProduct }: Props) {
  ...
  useEffect(() => {
    ...
    const nextUrl = "/shopify/products" + toQuery({ storeId, search, status: status !== "all" ? status : undefined, inventory: inventory !== "all" ? inventory : undefined, productType, vendor, sortKey, reverse: String(reverse), first: pageSize !== 25 ? pageSize : undefined, page: page > 1 ? page : undefined });
    const currentUrl = window.location.pathname + window.location.search;
    if (nextUrl !== currentUrl) window.history.replaceState({}, "", nextUrl);
  }, [inventory, page, pageSize, productType, reverse, search, sortKey, status, storeId, vendor]);
  ...
  const returnPath = listReturnPath({ storeId, search, status, inventory, productType, vendor, sortKey, reverse, pageSize, page });
  ...
  return (
    <section className="shopify-products-view">
      <header className="page-heading shopify-products-heading"><div><span>SHOPIFY CATALOG</span><h1>Shopify 商品</h1><p>浏览已绑定店铺的全部商品；编辑、翻译和媒体处理在独立详情页完成。</p></div><div className="shopify-products-heading-actions"><label className="shopify-store-picker"><span>当前店铺</span><select value={storeId} onChange={(event) => { setStoreId(event.target.value); setPage(1); }} disabled={!activeStores.length}>{activeStores.map((store) => <option key={store.id} value={store.id}>{store.displayName || store.shopDomain}</option>)}</select></label><button className="button primary" type="button" onClick={() => void fetchList(page, cursors[page] ?? null)} disabled={loading || !storeId}><RefreshCw className={loading ? "spin" : ""} size={16} />刷新</button></div></header>
      {!activeStores.length ? <div className="shopify-products-empty"><CircleAlert size={28} /><strong>还没有可用的 Shopify 店铺</strong><p>先在 Shopify 店铺页面完成应用凭据配置并测试连接。</p></div> : <div className="shopify-products-list">
        <section className="shopify-products-toolbar"><label className="search-field"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索商品标题、SKU、handle 或标签" aria-label="搜索 Shopify 商品" /></label><div className="shopify-filter-row"><Filter size={15} /><select className="filter-status" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} aria-label="商品状态"><option value="all">全部状态</option><option value="ACTIVE">在售</option><option value="DRAFT">草稿</option><option value="ARCHIVED">已归档</option><option value="UNLISTED">未上架</option></select><input className="filter-type" value={productType} onChange={(event) => { setProductType(event.target.value); setPage(1); }} placeholder="商品类型" aria-label="商品类型" /><input className="filter-vendor" value={vendor} onChange={(event) => { setVendor(event.target.value); setPage(1); }} placeholder="供应商" aria-label="供应商" /><select className="filter-inventory" value={inventory} onChange={(event) => { setInventory(event.target.value); setPage(1); }} aria-label="库存"><option value="all">全部库存</option><option value="in_stock">有库存</option><option value="out_of_stock">缺货</option></select><select className="filter-sort" value={sortKey} onChange={(event) => { setSortKey(event.target.value); setPage(1); }} aria-label="排序"><option value="UPDATED_AT">最近更新</option><option value="CREATED_AT">创建时间</option><option value="TITLE">标题</option><option value="INVENTORY_TOTAL">库存</option><option value="PRODUCT_TYPE">商品类型</option><option value="VENDOR">供应商</option></select><select className="filter-page-size" value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); }} aria-label="每页条数"><option value={25}>25 / 页</option><option value={50}>50 / 页</option><option value={100}>100 / 页</option></select><button className="icon-button filter-direction" type="button" onClick={() => setReverse((value) => !value)} aria-label={reverse ? "降序" : "升序"} title={reverse ? "降序" : "升序"}><ArrowDownUp size={16} /></button></div></section>
