...
import type { OneBoundItemPreview, SearchTask, SearchTaskOptions, SearchTaskResult, SearchTaskRun, ShopifyStore } from "../types";
...
export function SearchTasksPage({
  ...
  return <section className="search-tasks-view">
    <header className="page-heading"><div><span>PRODUCT SOURCING</span><h1>采集任务</h1><p>从插件保存商品信息与图片，执行 1688 搜图，并将选中结果直接导入 Shopify。</p></div><div className="search-task-count">{hasFilters ? `${total} 个匹配任务` : `${total} 个任务`}</div></header>
    ...
    {loading && tasks.length === 0 ? <div className="search-task-state"><LoaderCircle className="spin" size={20} />正在加载采集任务</div> : tasks.length === 0 ? <div className="search-task-state"><Search size={22} /><strong>{hasFilters ? "没有符合条件的任务" : "暂无采集任务"}</strong><span>{hasFilters ? "尝试修改关键词或任务状态。" : "从浏览器插件创建采集任务后，会同步显示在这里。"}</span>{hasFilters && <button className="button quiet compact" type="button" onClick={clearFilters}>清除筛选</button>}</div> : <div className={`search-task-list ${loading ? "loading" : ""}`} aria-busy={loading}>{tasks.map((task) => {
      ...
      return <article className="search-task-card" key={task.id}>
        ...
        <section className="task-query-workbench" aria-label={`${task.name} 搜图配置`}>
          ...
          <div className="task-query-config"><header><div><span>SEARCH CONFIG</span><strong>搜图参数</strong></div><small>本次消耗 20 积分</small></header><div className="task-query-fields">
          ...
          </div><button className="button primary task-query-submit" type="button" onClick={() => void runQuery(task)} disabled={isRunning || !queryDraft.imageId}>{isRunning ? <LoaderCircle className="spin" size={17} /> : <Search size={17} />}{isRunning ? "正在搜图" : task.runs.length ? "发起新一轮搜图" : "开始搜图"}</button></div>
        </section>
          ...
          return <section className={`task-run task-run-group ${selectedRun.status}`} key={group.key}>
            ...
            {!collapsed && completed && <div className="search-task-results">{selectedRun.results.map((result, resultIndex) => {
              ...
              return <article className={`search-task-result ${resultImported ? "imported" : ""}`} key={`${selectedRun.id}-${result.offerId || resultIndex}`}>
                ...
                <div className="search-task-result-actions"><button type="button" onClick={() => result.offerId && void importResults(task.id, selectedRun.id, [result.offerId])} disabled={!result.offerId || resultImported || Boolean(importingKey) || !selectedStoreId} title={selectedStoreId ? "导入到 Shopify" : "请先选择 Shopify 店铺"}>{resultImported ? <CheckCircle2 size={14} /> : <PackagePlus size={14} />}{resultImported ? "当前店铺已导入" : importingKey === importKey ? "导入中" : "导入 Shopify"}</button><button type="button" onClick={() => void openDetail(task, result, selectedRun)} disabled={!result.offerId}><FileSearch size={14} />商品详情</button><button type="button" onClick={() => result.imageUrl && setCompareState({ originalUrl, resultUrl: result.imageUrl, title: result.title || result.offerId || "图片对比" })} disabled={!result.imageUrl}><Images size={14} />对比图片</button>{detailUrl && <a href={detailUrl} target="_blank" rel="noreferrer" aria-label="打开 1688 商品页" title="打开 1688 商品页"><ExternalLink size={14} />打开1688</a>}</div>
              </article>;
