...
Mailshop 是一个面向跨境电商选品的采集任务与 Shopify 管理工具。浏览器插件创建采集任务，服务端保存图片和 1688 搜图轮次，用户确认结果后直接导入 Shopify 商品。
...
项目已经具备可运行的采集任务到 Shopify 商品闭环，以及认证、存储、导入和测试基础。`shopify_stores` 表和同步字段为后续 Shopify App 安装、Webhook 和双向同步预留。
